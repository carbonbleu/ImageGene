...
decode_contour(uint8_t* flags, unsigned int basePoint, unsigned int count, struct outline* outl)
{
		...
		if (flags[i] & POINT_IS_ON_CURVE) {
			if (gotCtrl) {
				if (outl->numCurves >= outl->capCurves && grow_curves(outl) < 0)
					return -1;
				outl->curves[outl->numCurves++] = (struct curve){ (uint_least16_t)(beg), (uint_least16_t)(basePoint + i), (uint_least16_t)(ctrl) };
			}
			else {
				if (outl->numLines >= outl->capLines && grow_lines(outl) < 0)
					return -1;
				outl->lines[outl->numLines++] = (struct line){ (uint_least16_t)(beg), (uint_least16_t)(basePoint + i) };
			}
			beg = basePoint + i;
			gotCtrl = 0;
		}
			...
			if (gotCtrl) {
				...
				if (outl->numCurves >= outl->capCurves && grow_curves(outl) < 0)
					return -1;
				outl->curves[outl->numCurves++] = (struct curve){ (uint_least16_t)(beg), (uint_least16_t)(center), (uint_least16_t)(ctrl) };
	...
	if (gotCtrl) {
		if (outl->numCurves >= outl->capCurves && grow_curves(outl) < 0)
			return -1;
		outl->curves[outl->numCurves++] = (struct curve){ (uint_least16_t)(beg), (uint_least16_t)(looseEnd), (uint_least16_t)(ctrl) };
	}
	else {
		if (outl->numLines >= outl->capLines && grow_lines(outl) < 0)
			return -1;
		outl->lines[outl->numLines++] = (struct line){ (uint_least16_t)(beg), (uint_least16_t)(looseEnd) };
	}
...
#define STACK_SIZE 10
	...
	for (;;) {
		if (is_flat(outl, curve, 0.5) || top >= STACK_SIZE) {
			...
			stack[top++] = (struct curve){ curve.beg, (uint_least16_t)(pivot), (uint_least16_t)(ctrl0) };
			curve = (struct curve){ (uint_least16_t)(pivot), curve.end, (uint_least16_t)(ctrl1) };
		}
	}
	return 0;
#undef STACK_SIZE
